...
AFRAME.registerComponent("shoot", {
  ...
  collision:function(e){
    const scene = document.querySelector("#scene")
    const element = e.details.target.el
    const elementHit = e.details.body.el

    if(elementHit.includes("Bear")){
      player.currentCash += 20
      element.removeEventListener("collide")
      scene.removeChild(element)
      scene.removeChild(elementHit)
    }
    if(elementHit.includes("Llama")){
      player.currentCash += 16.5
      element.removeEventListener("collide")
      scene.removeChild(element)
      scene.removeChild(elementHit)
    }
    if(elementHit.includes("Elephant")){
      player.currentCash += 24
      element.removeEventListener("collide")
      scene.removeChild(element)
      scene.removeChild(elementHit)
    }
    if(elementHit.includes("Deer")){
      player.currentCash += 21
      element.removeEventListener("collide")
      scene.removeChild(element)
      scene.removeChild(elementHit)
    }
    if(elementHit.includes("Wolf")){
